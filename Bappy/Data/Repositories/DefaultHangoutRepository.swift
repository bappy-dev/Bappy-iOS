...
//                    let bool = responseDTO.data as! Bool
    ...
    func fetchLikedPeople(id: String) -> RxSwift.Single<Result<[String], Error>> {
        let endpoint = APIEndpoints.fetchLikedPeople(id: id)
        
        return provider.request(with: endpoint)
            .map { result -> Result<[String], Error> in
                switch result {
                case .success(let responseDTO):
                    return .success(responseDTO.data.nicknames)
                case .failure(let error):
                    return .failure(error)
                }
            }
