//
//  MakeReviewCompletedViewController.swift
//  Bappy
//
...
final class MakeReviewCompletedViewController: UIViewController {
    
    // MARK: Properties
    private let viewModel: MakeReviewCompletedViewModel
    ...
    private let captionLabel: UILabel = {
        let label = UILabel()
        label.text = "You made BAPPY a better community!\nDo you wanna check the reviews you got?"
    ...
    // MARK: Lifecycle
    init(viewModel: MakeReviewCompletedViewModel) {
...
// MARK: - Bind
extension MakeReviewCompletedViewController {
    private func bind() {
        okButton.rx.tap
        ...
        viewModel.output.moveToProfileView
            .compactMap { $0 }
            .emit(onNext: { tabBarViewModel in
                guard let sceneDelegate = UIApplication.shared.connectedScenes.first?.delegate as? SceneDelegate else { return }
                sceneDelegate.switchRootViewToMainView(viewModel: tabBarViewModel)
            })
            .disposed(by: disposeBag)
    }
}
